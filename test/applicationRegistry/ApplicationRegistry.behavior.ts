...
export function shouldBehaveLikeApplicationRegistry(): void {
  ...
  it("deployer can set workspaceRegistry", async function () {
    ...
    expect(await this.applicationRegistry.workspaceRegApplicationRegistry()).to.equal(this.workspaceRegistry.address);
  });

  // it("active grant can receive application", async function () {
  //   const grantDeploy = await this.grantPrep.deploy(
  //     0,
  //     "dummyGrantIpfsHash",
  ...
  //   );
  //   this.grant = grantDeploy.deployed();
  //   console.log("Grant deployed:", this.grant);
